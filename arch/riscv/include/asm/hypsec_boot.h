#ifndef __RISCV_STAGE2_BOOT__
#define __RISCV_STAGE2_BOOT__

#define VM_LOADER_START		0x40000000
#define KERNEL64_LOAD_ADDR	0x00080000

#define HS_MAX_VMID		65
//Hostvisor:0 + 32 VMIDs + 1 COREVISOR
#define HS_VM_INFO_SIZE	HS_MAX_VMID + 1
...
extern int hs_set_boot_info(u32 vmid, unsigned long load_addr,
			unsigned long size, int image_type);
extern int hs_remap_vm_image(u32 vmid, unsigned long pfn, int id);
extern int hs_verify_and_load_images(u32 vmid);
...
int __hs_set_boot_info(u32 vmid, unsigned long load_addr,
			 unsigned long size, int image_type);
void __hs_remap_vm_image(u32 vmid, unsigned long pfn, int id);
bool __hs_verify_and_load_images(u32 vmid);
void __hs_boot_from_inc_exe(u32 vmid);

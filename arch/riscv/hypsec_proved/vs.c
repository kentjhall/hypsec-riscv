...
void init_hs_data_page(void)
{
	...
	hs_data->smmu_lock = (arch_spinlock_t)__ARCH_SPIN_LOCK_UNLOCKED;
	...
	hs_data->smmu_page_pool_start = (u64)__pa(smmu_pgs_start);
	hs_data->smmu_pgd_pool = hs_data->smmu_page_pool_start;
	hs_data->smmu_pmd_pool = hs_data->smmu_page_pool_start + SMMU_PMD_BASE;
	...
	for (i = 0; i < HS_SMMU_CFG_SIZE; i++) {
		hs_data->smmu_cfg[i].hw_ttbr = host_alloc_stage2_page(2);
		hs_data->smmu_cfg[i].vmid = V_INVALID;
	}
...
void init_hypsec_io(void)
{
	int i = 0, err;
	struct hs_data *hs_data;
	struct hs_arm_smmu_device *smmu;
	...
	for (i = 0; i < hs_data->hs_smmu_num; i++) {
		smmu = &hs_data->smmus[i];
...
/* DMA Protection */
void hs_smmu_free_pgd(u32 cbndx, u32 num)
{
	kvm_call_core(HVC_SMMU_FREE_PGD, cbndx, num);
}

void hs_smmu_alloc_pgd(u32 cbndx, u32 vmid, u32 num)
{
	kvm_call_core(HVC_SMMU_ALLOC_PGD, cbndx, vmid, num);
}

void hs_arm_lpae_map(u64 iova, phys_addr_t paddr, u64 prot, u32 cbndx, u32 num)
{
	kvm_call_core(HVC_SMMU_LPAE_MAP, iova, paddr, prot, cbndx, num);
}

phys_addr_t hs_arm_lpae_iova_to_phys(u64 iova, u32 cbndx, u32 num)
{
	return kvm_call_core(HVC_SMMU_LPAE_IOVA_TO_PHYS, iova, cbndx, num);
}

void hs_smmu_clear(u64 iova, u32 cbndx, u32 num)
{

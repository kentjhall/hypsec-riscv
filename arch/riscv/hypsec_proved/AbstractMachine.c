...
void clear_shadow_gp_regs(u32 vmid, u32 vcpuid) {
	...
	el2_memset(&el2_data->shadow_vcpu_ctxt[offset].ctxt, 0, sizeof(struct kvm_cpu_context));
}
...
void dump_output_el2(uint8_t *out, int len)
{
	int i;
	unsigned long s = 0;
	for (i = 0; i < len; i++) {
...
void test_aes(struct el2_data *el2_data)
{
	uint8_t sbox[256];
...
void reset_fp_regs(u32 vmid, int vcpu_id)
{
	...
	el2_memcpy(&shadow_ctxt->ctxt.fp, &kvm_cpu_context->fp,
		...
		struct kvm_cpu_context *kvm_cpu_context)
{
	...
	el2_memcpy(kvm_cpu_context, out_gpr, sizeof(__u64) * 35);

	encrypt_buf(vmid, (u64)&kvm_cpu_context->fp, (u64)&fpsimd, sizeof(union __riscv_fp_state));
	el2_memcpy(&kvm_cpu_context->fp, &fpsimd, sizeof(union __riscv_fp_state));
...
void encrypt_gp_regs(u32 vmid, u32 vcpu_id)
{
	...
	uint64_t *p;
	shadow_ctxt = hypsec_vcpu_id_to_shadow_ctxt(vmid, vcpu_id);
	el2_memcpy(&gp_local, &shadow_ctxt->ctxt, sizeof(struct kvm_cpu_context));
	...
	el2_memcpy(&vcpu->arch.guest_context, &gp_local, sizeof(struct kvm_cpu_context));
	/* for (i = 0; i < 31; i++) */
...
void decrypt_gp_regs(u32 vmid, u32 vcpu_id)
{
	...
	uint64_t *p;
	shadow_ctxt = hypsec_vcpu_id_to_shadow_ctxt(vmid, vcpu_id);
	el2_memcpy(&gp_local, &vcpu->arch.guest_context, sizeof(struct kvm_cpu_context));
	decrypt_kvm_cpu_context(vmid, &gp_local);
	//gp_local.regs.pstate = vcpu->arch.ctxt.gp_regs.regs.pstate;
	el2_memcpy(&shadow_ctxt->ctxt, &gp_local, sizeof(struct kvm_cpu_context));
	el2_memset(&vcpu->arch.guest_context, 0, sizeof(struct kvm_cpu_context));
	/* for (i = 0; i < 31; i++) */
	/* 	printhex_ul(shadow_ctxt->gp_regs.regs.regs[i]); */
	/* printhex_ul(shadow_ctxt->gp_regs.regs.sp); */
	/* printhex_ul(shadow_ctxt->gp_regs.regs.pc); */
...
void encrypt_sys_regs(u32 vmid, u32 vcpu_id)
{
	...
	el2_memcpy(&vcpu->arch.guest_csr, sr_local, SHADOW_SYS_REGS_LEN);
...
void decrypt_sys_regs(u32 vmid, u32 vcpu_id)
{
	...
	el2_memcpy(&shadow_ctxt->csr, sr_local, SHADOW_SYS_REGS_LEN);
	//el2_memcpy(&shadow_ctxt->sys_regs, &vcpu->arch.ctxt.sys_regs, SHADOW_SYS_REGS_LEN);
	for (i = 0; i < SHADOW_SYS_REGS_SIZE + 1; i++) {
		//printhex_ul(vcpu->arch.ctxt.sys_regs[i]);
		//printhex_ul(shadow_ctxt->sys_regs[i]);
	}
	el2_memset(&vcpu->arch.guest_csr, 0, SHADOW_SYS_REGS_LEN);
}

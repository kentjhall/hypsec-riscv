...
void init_el2_data_page(void)
{
	...
	for (i = 1; i < EL2_VM_INFO_SIZE - 1; i++) {
		...
		hgatp = (el2_data->vm_info[i].page_pool_start >> PAGE_SHIFT) & HGATP_PPN;
		el2_data->vm_info[i].hgatp = (hgatp | vmid64 | stage2_mode);
	...
	el2_data->host_hgatp = (el2_data->vm_info[HOSTVISOR].page_pool_start >> PAGE_SHIFT) & HGATP_PPN;
	el2_data->host_hgatp |= stage2_mode;
	el2_data->vm_info[HOSTVISOR].hgatp = el2_data->host_hgatp;
	...
	memcpy(el2_data->key, key, 16);
	memcpy(el2_data->iv, iv, 16);
...
phys_addr_t host_alloc_pte(unsigned int num)
{
	...
	stage2_spin_unlock(&el2_data->abs_lock);
	if (p_addr >= el2_data->host_hgatp) {
		printk("BUG: pte [start %lx paddr %lx host_hgatp %lx\n",
			(unsigned long)el2_data->vm_info[COREVISOR].page_pool_start,
			(unsigned long)p_addr, (unsigned long)el2_data->host_hgatp
			);
		BUG();
	}	
